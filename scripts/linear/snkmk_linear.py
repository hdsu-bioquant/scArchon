...
print(f'--------------- Start linear model ---------------')
train = adata[~(adata.obs[batch] == target)]
...
# Step 1: Compute average gene expression for 'control' and 'stimulated' in the train set
control_avg_train = train[train.obs[condition] == condition_control].X.mean(axis=0)
stimulated_avg_train = train[train.obs[condition] == condition_stimulated].X.mean(axis=0)

# Convert sparse matrices to dense format if needed
control_avg_train = np.asarray(control_avg_train).flatten()
stimulated_avg_train = np.asarray(stimulated_avg_train).flatten()

# Step 2: Calculate gene-wise difference
gene_diff_train = stimulated_avg_train - control_avg_train

print(f'--------------- Predicting ---------------')

# Step 3: Apply the gene differences to the test subset
predicted = ctrl_adata.copy()
predicted.X += gene_diff_train

stim_adata.obs[condition] = 'stimulated'
ctrl_adata.obs[condition] = 'control'
predicted.obs[condition] = 'predicted'

ctrl_adata.obs_names_make_unique()
stim_adata.obs_names_make_unique()
predicted.obs_names_make_unique()

ctrl_adata.var_names_make_unique()
stim_adata.var_names_make_unique()
predicted.var_names_make_unique()

adata_combined = stim_adata.concatenate(
    ctrl_adata,
    predicted)
...
adata_combined.write(f'{h5ad_dir}/{experiment_name}_linear_{target}.h5ad')

# Output flag
file_path = Path(f'flags/h5ad/output_run_flag_{experiment_name}_linear_{target}_h5ad.txt')
file_path.parent.mkdir(parents=True, exist_ok=True)
file_path.touch()

print('Linear model successfully saved the results')