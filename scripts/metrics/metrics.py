...
#### plot pca, umap and tsne ####
    ...
    ##### function for the differnetial enrichment #####
    def diff_GE(self, output_path = '../results', exp_index = 'exp1'):
        print("Computing differential gene enrichment")

        save_directory = os.path.join(output_path, f'diff_ge')
        ...
        degs_sti = list(self.adata.uns["rank_genes_groups"]["names"][self.keys['stimulated']][0:1000])
        degs_pred = list(self.adata.uns["rank_genes_groups"]["names"][self.keys['predicted']][0:1000])

        # getting the enrichment of the degs
        enr1 = gp.enrichr(gene_list=degs_sti, # or "./tests/data/gene_list.txt",
                    gene_sets=['MSigDB_Hallmark_2020'],
                    organism='human', # don't forget to set organism to the one you desired! e.g. Yeast
                    outdir=None, # don't write to disk
                )

        enr2 = gp.enrichr(gene_list=degs_pred, # or "./tests/data/gene_list.txt",
                    gene_sets=['MSigDB_Hallmark_2020'],
                    organism='human', # don't forget to set organism to the one you desired! e.g. Yeast
                    outdir=None, # don't write to disk
                )

        # tables of all the enrichments
        enr1.results.to_csv(f"{save_directory}/{exp_index}_enrich_table_stimulated.csv", sep=",")
        enr2.results.to_csv(f"{save_directory}/{exp_index}_enrich_table_predicted.csv", sep=",")

        # table of the significant enrichments
        table_enr1 = enr1.results.loc[enr1.results["Adjusted P-value"] <= 0.05]
        table_enr1.to_csv(f"{save_directory}/{exp_index}_significativ_enrich_table_stimulated.csv", sep=",")

        table_enr2 = enr2.results.loc[enr2.results["Adjusted P-value"] <= 0.05]
        table_enr2.to_csv(f"{save_directory}/{exp_index}_significativ_enrich_table_predicted.csv", sep=",")

        # number of elements in each significativ enrichment tables
        number_stim =len(table_enr1["Term"])
        number_pred =len(table_enr2["Term"])
        number_common = len(list(set(table_enr1["Term"]).intersection(set(table_enr2["Term"]))))

        # Calculate False Positives and False Negatives
        FP = number_pred - number_common  # Terms in table_enr2 but not in table_enr1
        FN = number_stim - number_common  # Terms in table_enr1 but not in table_enr2
        TP = number_common  # Terms that are in both lists
        ...
        if number_stim == 0:
            percent_common_over_true_diff_ge = 0
        else:
            percent_common_over_true_diff_ge = number_common / number_stim



        category = list(['Stimulated', 'Predicted', 'Common'])
        numbers = list([number_stim, number_pred, number_common])

        table_hallmarks_numbers = pd.DataFrame(
        {'category': category,
        'numbers': numbers
        })
        table_hallmarks_numbers['numbers'] = table_hallmarks_numbers['numbers'].astype(float)
        table_hallmarks_numbers['category'] = table_hallmarks_numbers['category'].astype(str)


        plt.bar(table_hallmarks_numbers['category'], table_hallmarks_numbers['numbers'], color="steelblue")
        plt.title('number of enriched terms')
        plt.xlabel('category')
        plt.ylabel('numbers')
        plt.savefig(f'{save_directory}/{exp_index}_shared_enriched_genes.png')
        plt.close()
        ...
        try:
            ...
            ax1 = dotplot(enr1.results,
                ...
                marker='o',
                ofname=f"{save_directory}/{exp_index}_enrich_dotplot_stimulated.pdf"
            )
        except:
            print(f"The first thousand genes of the stimulated data of {exp_index} is not enriched")

        try:
            # categorical scatterplot
            ax2 = dotplot(enr2.results,
                column="Adjusted P-value",
                x='Gene_set', # set x axis, so you could do a multi-sample/library comparsion
                size=10,
                top_term=5,
                ...
                show_ring=True, # set to False to revmove outer ring
                marker='o',
                ofname=f"{save_directory}/{exp_index}_enrich_dotplot_predicted.pdf"
            )
        except:
            print(f"The first thousand genes of the predicted data is not enriched")

        # Log into results file
        self.distance_scores['common_diff_ge'] = number_common
        self.distance_scores['recall_diff_ge'] = recall
        self.distance_scores['precision_diff_ge'] = precision
        self.distance_scores['f1_diff_ge'] = f1_score
        self.distance_scores['percent_common_over_true_diff_ge'] = percent_common_over_true_diff_ge
